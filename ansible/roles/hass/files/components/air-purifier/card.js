...
const styles = html`
  <style>
    ...
    .preview {
      background: transparent no-repeat center url('/local/components/air-purifier/working.gif');
      height: 220px;
    ...
    .preview.idle {
      background-image: url('/local/components/air-purifier/standby.gif');
      background-size: 280px 280px;
    }
...
class AirPurifierCard extends LitElement {
  ...
  callService(platform, service, options = {}) {
  ...
  renderHeader() {
    ...
    return html`
      <div class="stats-block">
        <span class="stats-hours">${temperature}</span> <sup>°C</sup>
        <div class="stats-subtitle">Temperature</div>
      </div>
      <div class="stats-block">
        <span class="stats-hours">${humidity}</span> <sup>%</sup>
        <div class="stats-subtitle">Humidity</div>
      </div>
      <div class="stats-block">
        <span class="stats-hours">${illuminance}</span> <sup>LUX</sup>
        <div class="stats-subtitle">Illuminance</div>
      </div>
    `
  }
    ...
    return html`
      <div class="toolbar">
        <paper-icon-button  icon="mdi:power-standby"
                            ...
                            @click='${(e) => this.callService('fan', 'toggle')}'>
        </paper-icon-button>
        ...
        <paper-icon-button  icon="mdi:circle-slice-3"
                            title="30%"
                            class="toolbar-item ${mode == 'favorite' && favorite_level == 4 && 'toolbar-item-on'}"
                            @click='${(e) => this.setFavorite(4)}'>
        </paper-icon-button>
